...
describe("ModelContextBadge", () => {
  it("shows the family label + version + ctx% for a known Claude model (70k / 200k = 35%)", () => {
    const w = mountBadge({ model: "claude-opus-4-20250514", contextTokens: 70_000 });
    expect(w.find('[data-testid="model-badge"]').text()).toBe("Opus 4 · ctx 35%");
  });

  it("maps sonnet and haiku ids to their short labels", () => {
    expect(mountBadge({ model: "claude-3-5-sonnet-20241022", contextTokens: 0 }).find("span").text()).toBe("Sonnet 3.5 · ctx 0%");
    expect(mountBadge({ model: "claude-haiku-4-20250101", contextTokens: 20_000 }).find("span").text()).toBe("Haiku 4 · ctx 10%");
  });
  ...
  it("uses a 1M window for current-generation models (Opus 4.6+, Sonnet 4.6+, Fable/Mythos)", () => {
    ...
    expect(mountBadge({ model: "claude-opus-4-8", contextTokens: 999_606 }).find("span").text()).toBe("Opus 4.8 · ctx 100%");
    expect(mountBadge({ model: "claude-sonnet-5", contextTokens: 500_000 }).find("span").text()).toBe("Sonnet 5 · ctx 50%");
    expect(mountBadge({ model: "claude-sonnet-4-6", contextTokens: 100_000 }).find("span").text()).toBe("Sonnet 4.6 · ctx 10%");
    expect(mountBadge({ model: "claude-fable-5", contextTokens: 250_000 }).find("span").text()).toBe("Fable 5 · ctx 25%");
  });

  it("keeps the 200k window for older Opus/Sonnet (pre-4.6)", () => {
    expect(mountBadge({ model: "claude-opus-4-5-20251101", contextTokens: 100_000 }).find("span").text()).toBe("Opus 4.5 · ctx 50%");
    expect(mountBadge({ model: "claude-sonnet-4-5-20250929", contextTokens: 100_000 }).find("span").text()).toBe("Sonnet 4.5 · ctx 50%");
  });
  ...
  it("hide-context (iTerm2 mode strip): model + version only, ctx% suppressed, tokens stay in the tooltip", () => {
    const w = mount(ModelContextBadge, {
      props: { agent: "claude", model: "claude-opus-4-8", contextTokens: 500_000, hideContext: true },
    });
    expect(w.find('[data-testid="model-badge"]').text()).toBe("Opus 4.8");
    expect(w.find('[data-testid="model-badge"]').attributes("title")).toContain("500,000");
  });
  ...
  it("rounds the percentage", () => {
    ...
    expect(mountBadge({ model: "claude-opus-4", contextTokens: 51_234 }).find("span").text()).toBe("Opus 4 · ctx 26%");
  });
