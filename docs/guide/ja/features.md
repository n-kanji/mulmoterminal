...
| 機能 | 説明 |
|---|---|
| 並行ターミナル | 1 ページ最大 **12 セル**、あふれると**ページ（タブ）**が増える。状態順の自動ソート（要対応が先頭） |
...
| ファイル添付 | ファイルをターミナルへ**ドラッグ & ドロップ**、またはヘッダーの添付ボタンの OS ファイルダイアログで**絶対パスを挿入** |
| 会話を読む | ヘッダーの本ボタンで**ツールログを畳んだ会話ビュー**を開く — プロンプトは引用、応答は Markdown レンダリング、応答間のジャンプボタン付き。長いターンでログの下に埋まった途中の応答へ戻る手段 |
